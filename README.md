...
>
> ✅ 已适配最新版 DSH **0.1.2-rc.1**（含新版浏览器 token 鉴权，打开 Web UI 自动带最新 token）。
...
- **适配 DSH 0.1.2+ 的浏览器鉴权（token）**：DSH 0.1.2 起 Web UI 需要 token 才能访问，启动器每次打开前都会实时读取最新日志里带 token 的 URL（dsh 被启动器之外重启过也没关系），不会再碰到 "authentication required"；登录一次后 30 天内可免 token 直接访问。
